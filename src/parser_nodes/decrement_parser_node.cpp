...
Decrement_Parser_Node::Rest::Rest(std::wstring&& keyword,
...
void Decrement_Parser_Node::Rest::dump(std::wostream& output) const
{
  output << m_keyword;
  for (const auto& n: m_noise) { n->dump(output); }
}

Decrement_Parser_Node::Decrement_Parser_Node(std::wstring&& keyword,
...
Decrement_Parser_Node::Rest_Ptr
Decrement_Parser_Node::parse_rest(Token_Stream& input)
{
  auto offset = input.tellg();

  auto keyword = check_and_remove(input, L"down");
  if (keyword.empty()) {
    input.seekg(offset);
    return nullptr;
  }
...
Decrement_Parser_Node_Ptr
Decrement_Parser_Node::parse(Token_Stream& input)
{
  auto offset = input.tellg();

  auto keyword = check_and_remove(input, L"knock");
  if (keyword.empty()) {
    input.seekg(offset);
    return nullptr;
  }
  ...
  return std::make_shared<Decrement_Parser_Node>(std::move(keyword),
...
void Decrement_Parser_Node::dump(std::wostream& output) const
{
  output << m_keyword;
...
void Decrement_Parser_Node::Rest::generate_cpp(Scope_State_Ptr state) const
{
  state->add_code(L" -- ");
}

void Decrement_Parser_Node::generate_cpp(Scope_State_Ptr state) const
{
  m_variable->generate_cpp(state);
  for (const auto& r: m_rest) { r->generate_cpp(state); }
}

void Decrement_Parser_Node::collect_variables(Scope_State_Ptr state) const
{
